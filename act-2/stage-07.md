...
Declare the ladder in the pin file stage 06 created, and declare it the way that file demands: **derived, never guessed**. The minors are not this page's to state, because AKS's window moves and this page does not; the authority is AKS's own release calendar ([rule 4.3](../rules.md#43-the-version-policy-follow-the-authority-at-the-pace-kubernetes-sets)). `scripts/derive-ladder` scrapes the calendar for the three minors AKS supports today and asks the kindest/node registry for the newest patch image of each. Read what it derived; the numbers below are the window as this stage was written, and yours are today's:
