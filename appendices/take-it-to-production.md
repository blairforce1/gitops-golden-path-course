...
| `scripts/style-gate`, `scripts/style-fix` | kustofmt then yamllint; rewrite in place | stage 11 | pinned container images; `.yamllint.yaml` at the root | nothing | yes |
...
| `scripts/check` and `check-*-parity`, `check-version-ladder` | every tool pinned to its one master; the Kubernetes ladder | stages 00, 07 | AKS as the flux master | change the master if your cloud differs; keep the graph walk | yes |
