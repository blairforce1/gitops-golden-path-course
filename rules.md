...
The quick reference at the end lists every rule on one line with what enforces it and the decision record behind it; the records, with the alternatives each rule rejected, are in [decisions/](seed/decisions/README.md), and stage 00 seeds them into your config repo beside the scripts. The same material read the other way, each thing the platform can do and the rules that build it, is [what the rules buy](appendices/what-the-rules-buy.md).
...
| **forge** | The service around git: the repository host with its pull requests, work items, checks, rulesets and deploy keys. GitHub here | The design asks nothing of it that GitLab or Gitea do not offer, and git is the truth while the forge is the service; naming them apart keeps every claim about evidence pointed at the right one |
...
**Why.** In a config repo the PR *is* the unit of review: it carries the approvals, the rendered diff, the status checks and the SLO evidence a promotion was gated on. A merge commit is the only strategy that leaves a durable pointer to it: `git log --first-parent` becomes the shipped-changes view, the full log the change-level view, and both survive. GitHub's default squash rewrites the subject (appending ` (#N)`, concatenating branch messages) so a commit written to the convention becomes a fiction that never reaches `main`. A squash set to `squash_merge_commit_title=PR_TITLE` and `squash_merge_commit_message=PR_BODY` keeps the subject, the body and the trailers, and still loses two things this course leans on: the two views of history (per PR and per commit, [stage 26](act-6/stage-26.md); a squash collapses them into one) and the branch commits a cluster may have applied (stage 25's ephemeral tenants). Rebase rewrites shas, so the revision a cluster reported an hour ago (`lastAppliedRevision`) can become unreachable at exactly the moment the release-notes range needs it. And GitHub's *default* merge subject is `Merge pull request #1 from owner/branch`: the PR title lands on line 3, so without `merge_commit_title=PR_TITLE` the merge commit itself violates the convention no matter how carefully the PR was titled.
...
**Where.** Stated at stage 01 (the plain manifests already follow it); enforced by `layout-gate` (one resource per file, the name shape against the kind and `metadata.name` inside, the folder), which `checkpoint-02` runs from stage 02 and `style-gate` runs first from stage 11, so the pre-commit hook and CI judge it too.
...
**How.** Exemptions: stage 01's flat `deploy/` (deliberately pre-convention; naive is its point) and tool-managed trees (Flux's `flux-system/`). `layout-gate` judges the taxonomy: a resource file outside `resources/`, `patches/`, `replacements/` or `secrets/` is a FAIL; `config/` holds generator inputs and is not judged.
...
**How.** `kustomize cfg fmt` was removed in kustomize v5 but the library survives: [kustofmt](https://github.com/blairforce1/kustofmt) wraps it as a formatter (a pinned binary, the pin in `clusters/versions.yaml` held to the kustomize pin's kyaml by `check-kustofmt-kustomize-parity`; `scripts/style-gate` checks, `scripts/style-fix` rewrites), so the house style is a byte comparison against what kyaml emits rather than a description. yamllint is the backstop for what formatting can't express. Editor settings can't express sequence indentation; don't try.
...
**Where.** Stage 00 (the toolchain gate), stage 03 (the flux pin), stage 05 (helm), stage 06 (sops/age), stage 07 (the version ladder), stage 11 (kustofmt), stage 16 (climbing it).

### 4.2 Tool provisioning: filters from images, operators installed

**The rule.** A tool that is a pure function over files/stdin with no credentials, cluster access or local state (kubeconform, conftest, trivy, yamllint) runs as a **container with a pinned tag**. A tool that holds credentials (gh), talks to the cluster (kubectl, flux), manages local machine state (kind, podman), writes files constantly (kustomize, kustofmt) or is an SDK (dotnet) installs natively, pinned to its authority where it has one. A tool that is both, a filter under one flag and a writer under another, installs as the writer: the operator side wins, which is why kustofmt (a filter under `-l`, a writer under `-w`, and the pre-commit hook on every commit) is a binary pinned in `clusters/versions.yaml` and not a container start on the commit path. Borderline constant-use filters (yq) go native for ergonomics.

**Why.** Environment parity is a property, not an aspiration, and the two halves deliver it differently. A filter's pin is the image tag in the command, so there is nothing to install and nothing to drift, and "works locally, fails in CI" is structurally impossible for that check. An operator's pin lives in `clusters/versions.yaml`, and `check` on the laptop and the install step in CI hold every venue to it (4.1). What the container buys a filter is that its pin cannot be left in a README.

**Where.** Stage 00; every gate from stage 02 on follows it.

### 4.3 The version policy: follow the authority, at the pace Kubernetes sets

**The rule.** Versions are *followed*, never chased: every pin (4.1) tracks its authority's current release, and the fleet's Kubernetes version climbs a **ladder**: prod up to dev's minor once dev has soaked it, dev up to platform's, then platform to the new one when the authority offers it; promotions on soak, the pin last, each class at most one minor behind the next and every class inside the authority's window, which is the deadline and not the trigger. The ladder is recorded in `clusters/versions.yaml` and climbed one rung per change. The local Flux mirrors the release bundled in AKS's `microsoft.flux` extension; OSS Flux typically runs a minor ahead of it, so the reflex `curl | bash` install grabs a version *ahead of the fleet*, the wrong direction.

**Why.** This is the same discipline the Kubernetes ecosystem itself runs on: the API is versioned, every tool tolerates a bounded version **skew** (kubectl supports one minor either side of the server; node components trail the control plane within a bound), and upgrades walk one minor at a time. Following the authority at that pace keeps the fleet inside every tool's tested skew window at all times. And the class split gives the policy its rungs for free, because **platform → dev → prod is a natural staging order for versions too**: a Kubernetes minor lands on the platform cluster, soaks, climbs to dev, then to prod, and the same order carries chart bumps and controller upgrades (5.11). The three-minor spread also matches what managed Kubernetes will hold you to: AKS supports N..N-2, so a fleet on next/current/stable sits exactly inside the supported window when Act VIII absorbs it - the ladder practised locally is the one the cloud enforces.
...
Ready is a resource fact; an SLO is a user fact, and the course's own app proves they diverge: `/healthz` is static, so a bad storage config is Ready-and-green while every real request fails. Promotion therefore requires **two signatures**: convergence (dev's green context) and performance (a clean SLO window on the rung below, `slo-gate`). No traffic is a FAIL: absence of evidence is not health. The soak behind the second signature is a number in git: `soak.yaml` declares it per artifact kind and target rung, `soak-gate` holds every promotion to its minimum from git's own dates, and an ascent that cannot wait carries a `Soak-waived:` trailer and passes loudly. The same file bounds the lag the rung below may carry, and `check-ladder-due` reports every ascent in progress against both numbers, red past the maximum, since a rung left too far behind is one the rung above no longer tests for. Security signals are a third evidence class on different clocks, deliberately not in the gate (stage 30 demonstrates them). *Stage 09; stage 16 (the soak declared); Act III checkpoint drill 2.*
...
A gate prints `PASS`/`FAIL` with what it means and exits non-zero; bare unlabeled output is banned at checkpoints. Every gate is a pure function over the working tree where it can be, so the *same file* runs as a paste block, in CI, and from a git hook. Each member of the `*-gate` family turns one class of evidence into one verdict: `layout-gate`, `policy-gate`, `style-gate`, `commit-gate`, `path-gate`, `base-gate`, `revert-gate`, `freeze-gate`, `slo-gate`, `soak-gate`. From stage 11 the hooks run them locally, and a red CI run on a *mechanical* check is a process failure: the hook should have caught it. *Stage 02 (first checkpoint), stage 08 (first gate), stage 11 (hooks).*
...
| 3.1 | File naming | `<name>.<kind>.yaml`, one resource per file | `layout-gate` (via `checkpoint-02`, `style-gate`) | [0007](seed/decisions/0007-one-resource-per-file-typed-folders.md) |
| 3.2 | Folders | typed folders; `secrets/` is a debt register | `layout-gate` (via `checkpoint-02`, `style-gate`) | [0007](seed/decisions/0007-one-resource-per-file-typed-folders.md) |
...
| 4.1 | Render rule | one authority per tool; kubectl never renders | `scripts/check` | [0010](seed/decisions/0010-one-authority-per-tool-kubectl-never-renders.md) |
...
| 4.3 | Version policy | follow the authority, skew bounded as Kubernetes' own; versions climb the class ladder | `scripts/check`, `versions.yaml` | [0012](seed/decisions/0012-follow-the-authority-at-kubernetes-pace.md) |
...
| 5.4 | Ladder is a practice | entry automatic, ascent by PR; off-rung paths need owners; base absorbs only what every rung already renders | `path-gate`, CODEOWNERS, `base-gate` | [0017](seed/decisions/0017-automation-writes-only-at-the-entry-rung.md) |
...
| 5.7 | Converged is not working | two signatures for promotion; the soak declared, min and max | `slo-gate`, `soak-gate`, `check-ladder-due`, `soak.yaml` | [0021](seed/decisions/0021-slo-gate-is-the-second-signature.md), [0040](seed/decisions/0040-the-soak-is-declared-in-git-and-judged-from-git-dates.md) |
